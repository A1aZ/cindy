/**
 * `/clear` handler 里三步的**先后契约**:立墓碑 → cleared_at 落库 → 推进 clear 纪元。
 *
...
describe('/clear:墓碑 → cleared_at 落库 → 推进 clear 纪元', () => {
  ...
  it('三步都在同一个 handler 内', () => {
    ...
    expect(handler).toContain('agentHandoffPending.markClearBoundarySettled(sid)');
  });
  ...
  it('顺序不可颠倒:纪元推进排在 cleared_at 落库之后,墓碑排在最前', () => {
    ...
    const epochAt = handler.indexOf('agentHandoffPending.markClearBoundarySettled(sid)');
    expect(tombstoneAt).toBeLessThan(persistAt);
    expect(persistAt).toBeLessThan(epochAt);
  });

  it('落库失败不跳过纪元推进:catch 掉而不是让 handler 抛出', () => {
    // 抛出的话 markClearBoundarySettled 不会执行,在途那批按过期历史算出的交接
    // 就能顺利写回。失败时宁可一并丢弃。
    const persistAt = handler.indexOf('await clearSessionContextInDb(sid, clearBoundaryMs)');
    const epochAt = handler.indexOf('agentHandoffPending.markClearBoundarySettled(sid)');
