...
\`\`\`json
{
  ...
  "whenToUse": "需要生成图片、插画、配图、修图、P 图、改图时找我",  // 1–${GHOST_MANIFEST_SUMMARY_MAX_CHARS} 字,模型发现与判断的唯一专用字段:只写"什么场景下找我"的场景枚举,会被花名册完整展示;缺省时花名册兼容回落用 description,但不要依赖回落
...
\`whenToUse\` 最多 ${GHOST_MANIFEST_SUMMARY_MAX_CHARS} 字符,在限制内会被会话花名册完整展示。模型从花名册命中目标后,
