...
describe('maker:predict-prompt — 多窗口去重', () => {
  it('同一 session 同一 turn(DB updatedAt 不变)并发调用时第二个请求直接返回 null,避免重复付费', async () => {
    // 让第一次预测暂挂,模拟并发
  ...
  it('同一 session 新 turn(DB updatedAt 变化)的预测可以替换旧条目的进行中请求', async () => {
    let resolveFirst!: (value: string) => void;
  ...
  it('同一 session DB updatedAt 不变时拒绝重复请求,避免跨窗口重复付费', async () => {
    let resolveFirst!: (value: string) => void;
    h.predict.mockImplementationOnce(
      () => new Promise<string>((resolve) => { resolveFirst = resolve; }),
    );

    // 窗口 A 发起预测(DB updatedAt=1)并暂挂
    h.sessionRow = { remoteHostId: null, agentKind: 'cc', updatedAt: 1 };
    const first = invokePredict(VALID_REQUEST);
    await vi.waitFor(() => expect(h.predict).toHaveBeenCalledTimes(1));

    // 窗口 B 同时发起预测,DB updatedAt 仍为 1(同一 turn),应被拒绝
