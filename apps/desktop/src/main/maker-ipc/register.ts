    ...
    session.onEvent((event: AgentEvent) => {
      ...
      if (event.type === 'done' && event.source === 'claude-code') {
        ...
        if (
          ...
          const rawDelta =
            prevReportedCost === undefined ? 0 : computeCumulativeCostDelta(prevReportedCost, cumulative);
          void (async () => {
            ...
            const usageOnlyDetails = buildClaudeTurnUsageDetails(
              undefined,
              undefined,
              resolvedModel,
              undefined,
            ...
            const recordUsageOnly = async (
              details: ReturnType<typeof buildClaudeTurnUsageDetails> = usageOnlyDetails,
            ) => {
