...
describe('OrcaWorkflowRoute source invariants', () => {
  ...
  it('does not navigate the detached sidebar window to settings from the worker toolbar', () => {
    ...
    // 实现用 onOpenSettings={isSidebarWindow() || deviceId ? undefined : handleOpenSettings} 在调用处守卫：
    ...
    expect(workerPanelSource).toContain('onOpenSettings={isSidebarWindow() || deviceId ? undefined : handleOpenSettings}');
