...
export interface QuotaHoverCardProps {
  /** New shared usage-card model. */
  account?: UsageCardAccount;
  /** Legacy Claude snapshot compatibility for the billing PR's renderer callers. */
...
function SessionUsageSection({
  ...
  const totalKey = hasMixedBreakdown
    ...
    : sessionUsage.costIsSdkEstimate || sessionUsage.estimatedValueIsSdkEstimate
...
export function QuotaHoverCard({
  ...
  const resolvedAccount = account ?? buildClaudeUsageCard(snapshot ?? null, t);
  const { title, planLabel, windows, details = [], notices = [], emptyText, updatedAt } =
    resolvedAccount;
