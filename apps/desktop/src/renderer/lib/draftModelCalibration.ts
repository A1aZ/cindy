...
function markedDefaultModelIdForAgent(
  ...
  const flagAgents: readonly AgentKind[] = agent === 'pi' ? ['pi', 'claude-code'] : [agent];
    ...
    const flagged = (provider.models[agent] ?? [])
      .filter(
        (model) =>
          ...
          (model.newSessionDefault?.some((marker) => flagAgents.includes(marker)) ?? false) &&
 ...
 *      同一来源多个被标记时按 sortOrder 决胜。Pi 接受 v3 自己的标记，同时保留既有非 XD
 *      目录的 claude-code 投影标记。**没有任何模型被标记时这步为空转，行为回到 1/2**（非破坏性）。
...
export function pickConnectedModelForAgent(
  ...
  // 0. 目录/服务端显式标记的新对话默认(newSessionDefault)优先。Pi 接受 v3 自己的标记，
  //    同时保留既有非 XD 目录的 claude-code 投影标记。
