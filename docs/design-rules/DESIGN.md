...
**Usage chart interaction geometry (owner-approved 2026-09-08).** The usage-data-marks exception above names its two registered members — the heatmap cell `usage-heatmap-day` and the stacked column `usage-token-bar` (the `data-usage-mark` values carried by `UsageHeatmap.tsx` / `UsageTokenBars.tsx`). For these two members in Usage History, the bounded hover/focus/selection emphasis in [the component specification](./usage-history-charts.md) is registered here as part of their treatment: the non-hit-testing visible layer may expand by 2px (heatmap cell width/height, bar width), which fits inside the 3px grid gaps and does not change the grid pitch or any resting geometry; bar height, baseline, categorical color tokens, heatmap intensity and hit regions do not change. The 09-08 reference refinement permits only the token chart to fade non-selected bars while an in-window date is selected or the recent-seven-day range is highlighted, as specified by the component entry; the heatmap may use its registered neutral selection outline, bars stay without a pointer-selection outline, and keyboard focus stays independently visible. All four corner radii remain 2px throughout. This registration is scoped to these two members — it is not permission for other marks to scale or for an interaction wrapper to distort data.
