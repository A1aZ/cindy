...
- **Dual anchoring**: interactive fills that sit on the **page** anchor to the page (`surface-hover` tier); fills that sit on **cards/popovers** anchor to the card (light `#F6F6F1`, ≈6% below card — `settings-menu-bg-selected`, `perm-item-selected-bg`, `ask-badge-bg`, `cmd-palette-item-hover`, etc.; dark selected rows on cards use the menu-hover lift `#282828`, not the page-anchored `#1D1D1D` chip). A single token cannot serve both planes after the revision; do not "unify" them back.
