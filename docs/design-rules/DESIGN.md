...
> - **Usage History chart color (owner-approved 2026-09-08).** The registered usage heatmap uses a blue intensity scale. Daily token bars and their corresponding model-table swatches/share marks use five existing palette hues through `--usage-model-1` … `--usage-model-5`; unlisted models stay neutral. `--usage-heatmap-high` supplies the high end of the heatmap. These chart roles reuse the process-category palette by explicit adjudication, not by inheriting process/status semantics. Agent/task marks, text and control frames stay neutral. See [the component specification](./usage-history-charts.md); colors.ts remains the value source.
> - **Resource Usage Process Categories** — the Resource Usage table may color only its 14px process-type glyphs so dense rows can be scanned by category (user-requested 2026-08-06). The six category pairs are task Agent blue `#2563EB / #60A5FA`, Agent service purple `#7C3AED / #A78BFA`, main-process pink `#DB2777 / #F472B6`, renderer cyan `#0891B2 / #22D3EE`, GPU amber `#D97706 / #F59E0B`, and utility green `#059669 / #34D399` (Light / Dark). These colors encode process category, **not health or status**; row backgrounds, labels, metrics, selection and actions remain on the neutral system. Direct use is confined to `resource-usage` process glyphs; the explicitly registered Usage History aliases above may also reference these palette values. No other table or process UI inherits this permission. Tokens: `--process-agent-task-icon`, `--process-agent-service-icon`, `--process-main-icon`, `--process-renderer-icon`, `--process-gpu-icon`, `--process-utility-icon`.
...
**Usage chart interaction geometry (owner-approved 2026-09-08).** For `usage-heatmap-day` and `usage-token-bar` in Usage History, the bounded hover/focus/selection expansion in [the component specification](./usage-history-charts.md) is part of the approved member treatment. It supersedes a blanket prohibition on changing these members’ visible dimensions during interaction: heatmap width/height and bar width may expand; bar height, baseline, data colors, hit regions and resting geometry do not. All four corner radii remain 2px throughout. This is an explicit registration update, not permission for other marks to scale or for an interaction wrapper to distort data.
...
| `--usage-heatmap-high`, `--usage-model-1` … `--usage-model-5` | Existing process-category palette aliases; see colors.ts | Existing Light/Dark palette aliases; see colors.ts | Usage History heatmap, token bars and matching model-table marks only (2026-09-08). Category/intensity, never status. No existing token values or theme overrides change. |
