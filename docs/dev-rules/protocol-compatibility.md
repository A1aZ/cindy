...
- Desktop 本地技能管理用扫描条目 `id` 区分不同 scope / 项目中的同源记录；启停与卸载
  ...
  按相同规则去重；大小写敏感卷上的同名大小写变体也保守互斥。卸载遇到目录大小写
  变体时按实际文件身份匹配清单，并使用清单原始名称清理记录及自动同步偏好。
