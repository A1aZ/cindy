...
- 发现结果决定账号成员；同 ID 的供应商新资料会更新，旧静态值不能压住实报。
...
接收严格解析器不认识的字段。当前改动没有修改或部署服务端。

本地离线目录已把现有 OpenAI/Gemini/xAI 图像、xAI 视频资料纳入 Registry，并补入
已有音频、识别、实时与向量型号的公共资料。公共音频/向量资料不虚构订阅接入路由。
请求尺寸、时长、音色、返回格式仍归现有执行协议；本次没有新增通用媒体参数表单，
也没有让尚未实现的音频协议自动变得可调用。
