...
未自定义时，伙伴直接复用客户端新任务的默认组合选择器及其顺序、模型和引擎可用性判定，
不得另造伙伴专属的推荐型号或来源排序。首次创建取同一首选，fallback 顺序取该选择器的
后续可用组合；用户自定义伙伴候选链仍优先。生成的默认候选不写成用户 override。
未连接或没有可用默认组合时保持空值，不写入 GLM / Gateway 占位；伙伴创建和打开入口
复用客户端现有连接引导，配置完成后重新解析，不能创建没有模型的主任务。
已有连接但推荐候选为空时，在创建前提供标准模型选择器；存量空模型伙伴打开后同样可直接选择模型恢复，不能只提供无效重试。
...
- `start_session_task`：创建一条真正独立运行的 Cindy 任务。它出现在用户主任务列表里，拥有
  ...
  需要独立交付和验证时使用。它没有 `target_bot_id`，也绝不以名为 Cindy 的伙伴代替。
