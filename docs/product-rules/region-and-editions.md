...
| 移动端设置里的区域标签 | `apps/mobile/src/settings/mobileSettings.ts:124` 的 debug 项输出 `Global` / `CN`。桌面侧栏已收敛：`apps/desktop/src/renderer/components/sidebar/UserInfoSection.tsx` 的 `REGION_LABEL` 只标 `cn` / `dev`，Global 构建的版本行只显示版本号 | §2.3、§2.5（`CN` 也不是本文规定的 `Mainland China`） | 属于调试 / 身份自查信息而非产品叙事表达，收敛优先级低于登录页；剩余的移动端 debug 项按同一口径（global 不标）收敛即可删除本行 |
