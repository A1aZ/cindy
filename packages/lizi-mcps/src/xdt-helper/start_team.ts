...
const DESCRIPTION =
  ...
  'worker_permission_mode 可选:auto(Auto-review)或 bypassPermissions(Full access);省略时沿用上次 Worker 创建偏好,首次使用默认 bypassPermissions。当前 session 已是 Lead 时,必须显式指定模式才能更新默认值。' +
  ...
  registry.register({
    ...
    inputShape: {
      worker_permission_mode: z
        ...
        .optional()
        .describe(
          'Worker 创建默认权限。省略时沿用已保存偏好（首次为 bypassPermissions）；当前 session 已是 Lead 时须显式指定才能更新默认值；指定后，UI 与后续 create_worker/create_workers 都沿用该模式。',
        ),
    },
