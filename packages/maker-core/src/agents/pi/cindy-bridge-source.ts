...
function bashExpandedPathCandidates(
  ...
  try {
    const paths = tilde.paths.flatMap((pattern) => {
      const matches = globSync(pattern, { cwd });
      if (matches.length === 0) {
        return [path.isAbsolute(pattern) ? pattern : path.resolve(cwd, pattern)];
      }
      return matches.map((match) => path.isAbsolute(match) ? match : path.resolve(cwd, match));
    });
...
function bashWorkingDirectoryCandidates(command: string): BashPathCandidates {
  ...
  for (const match of codeMask.matchAll(commandStart)) {
    ...
    for (const cwd of candidates) {
      const expanded = bashExpandedPathCandidates(parsed.target, cwd, parsed.mayExpand);
      unresolved ||= expanded.unresolved;
