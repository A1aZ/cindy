...
describe('对抗语料 — dotenv 凭证路径', () => {
  ...
  it.each([
    ...
    ["grep -r --include='[!.]*.ts' KEY .", 'auto-approve'],
    ["grep -r --include='config.env*' KEY .", 'auto-approve'],
    ...
    ['git diff -- README.md', 'auto-approve'],
    ['git diff HEAD -- README.md', 'auto-approve'],
    ['git diff -- src/index.ts', 'auto-approve'],
    ["git diff -- ':(top)README.md'", 'auto-approve'],
    ['git grep .env -- data.json', 'auto-approve'],
    ['git grep -- .env data.json', 'auto-approve'],
    ['git grep -e API_KEY -- README.md', 'auto-approve'],
