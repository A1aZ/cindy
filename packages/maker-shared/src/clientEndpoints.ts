 ...
 *    见 apps/desktop/src/main/endpointManifestCache.ts):只在**传输层**失败
 *    (超时 / DNS / 代理 / 5xx)时出现,缓存里存的是当初校验通过的清单原文,读回后
 *    仍走本模块的同一套严格解析,清单地址变化即作废,且端点主机必须落在**构建期
 *    烘焙的受信任域**内(缓存文件在 userData、可被其他进程写,语法合法 ≠ 来源可信)。JSON / schema / 非法值 /
 *    region 不匹配,以及永久性 HTTP 3xx/4xx(路径 / 权限 / 部署错)这类配置事故照旧
 *    硬阻断——本模块的解析语义一字未放松,宿主也不得据此新增任何自动回退。
